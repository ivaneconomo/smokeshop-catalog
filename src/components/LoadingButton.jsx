...
/**
 * Botón outline con spinner.
 * - Si pasas `loading` => modo controlado.
 * - Si NO pasas `loading` => el botón maneja loading mientras corre onClick.
 */
export default function LoadingButton({
  ...
  className = '',
  disabled = false,
  loading, // opcional (modo controlado)
  type = 'button',
  ...rest
}) {
  ...
  const classes =
    ...
    'dark:bg-gray-800/50 dark:text-slate-100/60 dark:border-gray-700/60 ' +
    ...
    >
      {isLoading && (
        <svg
          aria-hidden='true'
          role='status'
          className='inline w-4 h-4 me-2 animate-spin text-gray-200 dark:text-gray-600'
          viewBox='0 0 100 101'
          fill='none'
